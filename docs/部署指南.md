...
后端服务默认在 `https://localhost:888` 启动（端口与证书路径在 `backend/.env`）
...
前端服务默认在 `https://localhost:8888` 启动（Vite HTTPS 配置在 `frontend/vite.config.js`）
...
- 前端页面：`https://taitugou.top:888/`
- API：`https://taitugou.top:888/api/*`
- Socket.IO：`https://taitugou.top:888/socket.io`
