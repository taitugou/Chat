<template>
  <div class="chess-controls">
    ...
    <div v-if="gameType === 'weiqi'" class="grid grid-cols-1 gap-[1.5vh]">
      <div class="text-[1.9vh] text-white/50 text-center">点击棋盘落子</div>
    ...
    <div v-else-if="isBoardMoveGame" class="grid grid-cols-1 gap-[1.5vh]">
      <div v-if="isJunqiGame && gameState.phase === 'setup' && !junqiMyReady" class="space-y-[1.2vh]">
        <div class="text-[1.9vh] text-white/50 text-center">布阵阶段：点击“自动布阵”后进入对局</div>
        <div class="flex gap-[0.8vh] items-center">
          <div class="text-[1.8vh] text-white/40 whitespace-nowrap">种子</div>
          <input :value="junqiAutoSetupSeed" 
                 @input="$emit('update:junqiAutoSetupSeed', Number(($event.target as HTMLInputElement).value))"
                 type="number" min="1" class="glass-input flex-1 py-[0.8vh] text-[2.1vh]" />
        </div>
        <button class="glass-btn-primary py-[1.8vh] rounded-[1.2vh] font-black text-[2.3vh] active:scale-95 transition-all"
