<template>
  <div class="flex-1 flex flex-col overflow-auto h-full">
    <div class="flex items-center justify-between">
      <div class="text-[2.2vh] text-ios-label-tertiary">
      ...
      <div class="flex items-center gap-[3vw]">
        <div v-if="['zhajinhua', 'texas_holdem'].includes(gameType)" class="flex flex-col items-end">
          <div class="text-[1.8vh] text-ios-label-quaternary uppercase tracking-tighter">你的最低下注</div>
          <div class="text-[2.6vh] font-bold text-green-400">{{ myUnitBet }}</div>
        </div>
        <div class="flex flex-col items-end">
          <div class="text-[1.8vh] text-ios-label-quaternary uppercase tracking-tighter">当前总筹码</div>
    ...
    <div class="mt-[2vh] flex-1 flex flex-col items-center justify-center gap-[4vh]">
      ...
      <div v-if="gameType === 'blackjack' && Array.isArray(gameState.dealerHand) && gameState.dealerHand.length" class="flex flex-col items-center gap-[1vh]">
        <div class="text-[1.8vh] text-ios-label-quaternary uppercase tracking-widest">庄家</div>
      ...
      <div v-if="gameType === 'texas_holdem' && gameState.communityCards?.length > 0" class="flex flex-col items-center gap-[1vh]">
        <div class="text-[1.8vh] text-ios-label-quaternary uppercase tracking-widest">公共牌</div>
        <transition-group name="deal" tag="div" class="flex gap-[1.5vw]">
          <PokerCard
          ...
          <div v-for="n in (5 - gameState.communityCards.length)" :key="'empty-'+n"
               class="w-[8vh] h-[12vh] border border-ios-separator bg-ios-systemGray5 rounded-[1vh] flex items-center justify-center text-ios-label-quaternary text-[4vh]">
            ?
          </div>
        </transition-group>
      </div>
      ...
      <div class="flex flex-col items-center gap-[1vh]">
        <div v-if="gameType === 'texas_holdem'" class="text-[1.8vh] text-ios-label-quaternary uppercase tracking-widest">你的手牌</div>
        <div v-else-if="isCardSelectPlayGame" class="text-[1.8vh] text-ios-label-quaternary uppercase tracking-widest">
          {{ gameType === 'doudizhu' ? '手牌（多选）' : '手牌（多选）' }}
        </div>
        <div v-else-if="isShengjiGame" class="text-[1.8vh] text-ios-label-quaternary uppercase tracking-widest">
          手牌（单选）
        </div>

        <div v-if="displayHand.length > 0">
          <div class="flex justify-center mb-[1vh] gap-[1vh]" v-if="isCardSelectPlayGame || isShengjiGame">
             <button class="text-[1.8vh] text-ios-label-tertiary hover:text-ios-label-primary bg-black/20 px-[1vh] rounded" @click="sortHand('rank')">按点数排序</button>
             <button class="text-[1.8vh] text-ios-label-tertiary hover:text-ios-label-primary bg-black/20 px-[1vh] rounded" @click="sortHand('suit')">按花色排序</button>
          </div>
          <transition-group
            name="deal"
            tag="div"
          ...
          >
            <div
              ...
              :class="selectedCardIds.includes(String(card.id)) ? 'border-ios-blue/50 ring-2 ring-ios-blue/50 -translate-y-[2vh]' : ''"
            >
              <PokerCard
                :suit="card.suit"
                :value="card.rank"
        ...
        <div v-else class="text-center text-ios-label-quaternary">
    ...
    <!-- Last Play Info -->
    <div v-if="isCardSelectPlayGame" class="mt-[2vh] rounded-[1.5vh] border border-ios-separator bg-black/30 p-[1.5vh] text-[2.1vh] text-ios-label-secondary">
      ...
      <div v-else class="text-ios-label-tertiary">当前无上一手</div>
    </div>

    <!-- Shengji Info -->
    <div v-if="isShengjiGame" class="mt-[2vh] rounded-[1.5vh] border border-ios-separator bg-black/30 p-[1.5vh] text-[2.1vh] text-ios-label-secondary">
