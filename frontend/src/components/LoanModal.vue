<template>
  <div class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-ios-black/60 backdrop-blur-sm animate-fade-in" @click.self="$emit('close')">
    <div class="ios-modal w-full max-w-md max-h-[90vh] overflow-y-auto custom-scrollbar animate-modal-enter">
      <div class="p-6">
        ...
        <div class="mb-6 p-4 ios-card-content space-y-2">
          <div class="flex justify-between items-center">
            <span class="text-ios-label-secondary">当前余额</span>
            <span class="text-white font-bold">{{ userInfo?.chips || 0 }}</span>
          </div>
          <div class="flex justify-between items-center">
            <span class="text-ios-label-secondary">当前欠款</span>
            <span class="text-red-400 font-bold">{{ userInfo?.debt || 0 }}</span>
          </div>
          <div class="border-t border-ios-separator my-1 pt-1"></div>
          <div class="flex justify-between items-center">
            <span class="text-ios-label-secondary">当前额度</span>
            <span class="text-ios-blue font-bold">{{ maxLoan }}</span>
          </div>
          <div class="flex justify-between items-center">
            <span class="text-ios-label-secondary">可用额度</span>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-6">
          <div class="space-y-2">
            <label class="text-sm font-medium text-ios-label-secondary ml-1">借贷金额</label>
            <div class="grid grid-cols-3 gap-2">
              <button 
                ...
                class="ios-btn-secondary py-2 text-sm"
                :class="{ 'bg-ios-blue/40 border-ios-blue/60': selectedAmount === amount }"
              >
                {{ amount }}
              </button>
            </div>
            <div class="relative mt-2">
              <input 
                ...
                class="ios-input w-full pr-12"
                min="100"
                step="100"
              />
              <span class="absolute right-4 top-1/2 -translate-y-1/2 text-ios-label-tertiary text-sm">筹码</span>
            </div>
          </div>
